...
summary: 在Workload Factory中为数据库创建新的Microsoft SQL Server。 
---
= 在Workload Factory中为数据库创建数据库服务器
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
在数据库中创建新的Microsoft SQL Server或数据库主机需要部署FSx for ONTAP文件系统并为Active Directory提供资源。
...
--
Microsoft多路径I/O配置:: Microsoft SQL Server部署模式都要求使用iSCSI存储协议创建LUN。在通过FSx for ONTAP为SQL Server配置LUN的过程中、Workload Factory会配置Microsoft多路径I/O (MPIO)。MPIO根据AWS和NetApp最佳实践进行配置。
...
--
工作负载出厂操作模式:: 根据您对让Workload Factory管理AWS资源的习惯程度、Workload Factory可提供三种操作模式。
+
--
*_Basic_ mode*：在此操作模式下、您无需在Workload Factory中关联任何AWS帐户凭据。您可以从CodeBox复制或下载部分填充的YAML模板、以便在Workload Factory之外完成。

*_读取_模式*：在此操作模式下、您需要提供具有读取权限的AWS帐户凭据、以便填写_Quick cree_或_Advanced cree_表单、然后复制或下载该表单。您也可以使用填写好的表单详细信息从Workload Factory重定向到CloudFormation。您将能够在Workload Factory中管理已部署的数据库服务器。

*_自动_模式*：在此操作模式下、您可以提供具有自动权限的AWS帐户凭据、以便在Workload Factory中创建和管理AWS资源。
...
凭据和权限:: 您必须 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["AWS帐户凭据和自动模式权限"^] 在Workload Factory中创建新的数据库主机。
+
--
或者、您也可以使用Codebox复制空模板或创建完整的模板、以便使用REST API、AWS命令行界面或AWS CloudFormation在工作负载工厂外部部署数据库。link:https://docs.netapp.com/us-en/workload-setup-admin/codebox-automation.html["了解有关CodeBox自动化的更多信息"^](英文)
...
您可以使用_Quick create或_Advanced create部署模式在Workload Factory中以_Automate模式权限完成此任务。您还可以使用CodeBox中提供的以下工具：REST API、CloudFormation和Terraform。link:https://docs.netapp.com/us-en/workload-setup-admin/use-codebox.html#how-to-use-codebox["了解如何使用CodeBox实现自动化"^](英文)
...
+
通过具有_Automate权限的AWS凭据、Workload Factory可以通过您在Workload Factory中的AWS帐户部署和管理新的数据库主机。

+
通过具有_read_权限的AWS凭据、Workload Factory可以生成CloudFormation模板、以供您在AWS CloudFormation控制台中使用。

+
如果您在Workload Factory中没有关联的AWS凭据、而您希望在Workload Factory中创建新服务器、请按照*选项1*转到凭据页面。为数据库工作负载的_Automate_mode手动添加所需的凭据和权限。

+
如果要在Workload Factory中完成创建新服务器表单、以便下载完整的YAML文件模板以在AWS CloudFormation中进行部署、请按照*选项2*进行操作、以确保您具有在AWS CloudFormation中创建新服务器所需的权限。为数据库工作负载的_Read_模式手动添加所需的凭据和权限。

+
或者、您也可以从Codebox下载部分完成的YAML文件模板、以便在Workload Factory之外创建堆栈、而无需任何凭据或权限。从“代码”框的下拉列表中选择*CloudFormation*以下载YAML文件。
...
服务器部署完成后、Workload Factory不会在Microsoft SQL Server上启用远程连接。要启用远程连接、请完成以下步骤。
...
.下一步行动
现在您可以 link:create-database.html["在Workload Factory中创建数据库"]。
