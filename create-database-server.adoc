...
.凭据和权限
你必须 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["授予 _database 主机创建_ 权限"^] 在您的 AWS 账户中，在 Workload Factory 中创建一个新的数据库主机。
...
+
对于 GovCloud 帐户，请按以下格式输入 AWS SSM Parameter ARN 的值：
`{fsx:{"username":"fsxadmin","password":"pass"}}`。

+
该值在 AWS 管理控制台的参数详细信息下的参数存储中提供。AWS SSM 参数类型必须为"安全字符串"，名称格式为 `/netapp/wlmdb/<anyname>`。详细了解如何从link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文档"^]检索 SSM 参数。
...
+
对于 GovCloud 账户，请按以下格式输入 Active Directory 凭据的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 类型必须为"Secure String"，名称格式为  `/netapp/wlmdb/<anyname>`。了解有关从 link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文档"^] 检索 SSM 参数的更多信息。
...
+
对于 GovCloud 账户，请按以下格式输入 FSx for ONTAP 凭据的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 类型必须为"Secure String"，名称格式为  `/netapp/wlmdb/<anyname>`。了解有关从 link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文档"^] 检索 SSM 参数的更多信息。

+
新的FSx for ONTAP文件系统可能会增加30分钟或更长时间的安装时间。

... *选择现有的 FSx for ONTAP*：选择 FSx for ONTAP 名称，并为文件系统输入用户名和密码。
+
...
.. *安全组*：创建工作负载安全组，以允许在节点上进行 Microsoft SQL Server 和 Windows 群集通信所需的端口和协议。此外，您可以选择一个或多个安全组。
...
+
可选：选择“使用托管服务帐户”作为 SQL Server 服务帐户。如果您的环境使用 MSA（托管服务帐户）或组托管服务帐户 (gMSA)，并且密码管理由 Active Directory 处理，请使用此选项。
...
... 输入 Active Directory 域的用户名和密码。
+
对于 GovCloud 账户，请按以下格式输入 Active Directory 凭据的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 类型必须为"Secure String"，名称格式为  `/netapp/wlmdb/<anyname>`。了解有关从 link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文档"^] 检索 SSM 参数的更多信息。

... *首选域控制器*：（可选）输入要用于加入 Active Directory 的首选域控制器。
... *首选组织单元路径*：（可选）在 Active Directory 中输入要加入的首选组织单元 (OU)。
... *目标 Active Directory 组*：可选，输入要将计算机添加到的目标 Active Directory 组。
