...
在工作负载工厂中创建数据库需要_查看、规划和分析_权限。或者，您可以复制或下载部分完成的代码模板，在工作负载工厂之外完成操作。link:https://docs.netapp.com/us-en/workload-setup-admin/permissions-reference.html["了解工作负载工厂权限"^] 决定你想使用哪种模式。


NOTE: 使用SMB协议的Microsoft SQL Server不支持创建数据库。

.开始之前
在创建新数据库之前，请先完成以下先决条件。

* *凭据和权限*：您必须拥有 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["AWS 帐户凭据和_操作和补救_权限"^] 才能在 Workload Factory 中创建新数据库。
+
...
在 Workload Factory 中，您可以使用“快速创建”或“高级创建”部署模式来完成此任务。
