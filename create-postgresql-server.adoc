...
---
= 在 NetApp Workload Factory for Databases 中创建 PostgreSQL 服务器
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
你必须有 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["授予 _database 主机创建_ 权限"^] 在您的 AWS 账户中，在工作负载工厂中创建一个新的数据库主机。
...
.. *安全组*：创建工作负载安全组以允许 PostgreSQL 所需的端口和协议。此外，您可以选择一个或多个安全组。
...
+
对于 GovCloud 帐户，请按以下格式输入凭据的 AWS SSM 参数 ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM 参数类型必须为"安全字符串"，名称的格式为 `/netapp/wlmdb/<anyname>`。了解有关从 link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文档"^] 检索 SSM 参数的更多信息。
...
.. *标记*：您可以选择最多添加40个标记。
.. *Simple Notification Service*：您也可以从下拉菜单中选择 PostgreSQL 的 SNS 主题，为此配置启用 Simple Notification Service (SNS)。
+
... 启用简单通知服务。
... 从下拉菜单中选择ARN。
